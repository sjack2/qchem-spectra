...
parse_cli() {
    ...
    opts=$(getopt -o hb:m:c:g: \
        --long help,method:,basis:,ri:,grid:,scf-conv:,roots:,variant:,solvent:,solvent-model:,max-scf:,cpus:,\
...
write_qchem_input() {
    ...
    cat >"$inp_file" <<EOF
\$comment
TD-DFT UV-Vis + ECD (${solvent_desc}) -- generated by 6-qchem-tddft.sh
\$end
