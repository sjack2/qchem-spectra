...
ri_lines() {
    case $ri_mode in
        ...
        j|J)
            [[ ${basis,,} == def2-* ]] || \
                die "--ri j needs a def2-* basis (got '${basis}'); pass e.g. -b def2-TZVP. RI aux sets are auto-derived only for the def2 family"
            printf '\n  AUX_BASIS_J         RIJ-%s' "$basis" ;;
        jk|JK)
            [[ ${basis,,} == def2-* ]] || \
                die "--ri jk needs a def2-* basis (got '${basis}'); pass e.g. -b def2-TZVP"
...
parse_cli() {
    ...
    opts=$(getopt -o hb:m:c:g: \
        --long help,method:,basis:,ri:,disp:,grid:,variant:,solvent:,solvent-model:,max-scf:,scf-conv:,cpus:,\
...
write_qchem_input() {
    ...
    cat >"$inp_file" <<EOF
\$comment
IR + VCD (${solvent_desc}) -- generated by 6-qchem-vcd.sh
\$end
